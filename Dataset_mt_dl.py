...
class AShareParquetDataset(Dataset):
    def __init__(self, parquet_path, features_col, date_ids=None, target_col='LabelA', seq_len=5):
        self.parquet_path = parquet_path   # 现在这里是目录，如 D:/kaggle_data/train
...
class AShareTestDataset(Dataset):
    def __init__(self, parquet_path, features_col, seq_len=5):
        self.parquet_path = parquet_path   # 现在这里是目录，如 D:/kaggle_data/test
        self.features = features_col
        self.seq_len = seq_len
        self.date_ids = []
...
def generate_submission(model, test_parquet_path, feature_cols, seq_len=5, output_csv="submission.csv"):
    # test_parquet_path 现在是 test 目录
    ...
    with torch.no_grad(): # 关闭梯度计算
        for x, ids_numpy in tqdm(test_loader):
            ...
            for start_idx in range(0, num_stocks, chunk_size):
                ...
                y_pred_chunk = model(x_chunk) # shape: [Batch, 239, 3]
                
                # <--- 修改 4：只提取 LabelA (第 0 维) --->
                y_pred_chunk_A = y_pred_chunk[:, :, 0] # shape: [Batch, 239]
